...
<main>
    ...
    <table>
        ...
        <tr>
            <td class="desc"><h3>Total {{$cont}} categorias!</h3></td>
        </tr>
    </table>
    @foreach($categoriasPage as $categorias)
        <table border="0" cellspacing="0" cellpadding="0">
            ...
            @foreach($categorias as $categoria)
                <tr>
                    <td class="no">{{$categoria->id}}</td>
                    <td class="desc"><h3>{{$categoria->nome}}</h3></td>
                </tr>
            @endforeach
        </table>
        <footer>
            Copyright &copy; 2016 Os PrintF - <b>Páginas {{$i}} de {{$categoriasPage->count()}}</b>
        </footer>
        @if($i != $categoriasPage->count())
            <?php $i++ ?>
